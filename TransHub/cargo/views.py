...
from .forms import *
from .utils import *
from main.models import City, Region, Event
...
def load_regions(request):
    ...
    if country_id:
        ...
        return render(request, 'cargo/region_dropdown_list_options.html', {'regions': regions})
    return render(request, 'cargo/region_dropdown_list_options.html', {'regions': Region.objects.none()})
...
def load_cities(request):
    ...
    if region_id:
        ...
        return render(request, 'cargo/city_dropdown_list_options.html', {'cities': cities})
    return render(request, 'cargo/region_dropdown_list_options.html', {'regions': City.objects.none()})


class AddCargo(LoginRequiredMixin, DataMixin, CreateView):
    form_class = CargoForm
    template_name = 'cargo/add_cargo.html'
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        c_def = self.get_user_context(title='Додати вантаж')
        return dict(list(context.items()) + list(c_def.items()))

    def get_success_url(self):
        return reverse_lazy('cargo:my_cargo')


class AllCargo(LoginRequiredMixin, DataMixin2, ListView):
    paginate_by = 50
    model = Cargo
    template_name = 'cargo/all_cargo2.html'
    form_class = CargoFilterForm
    context_object_name = 'cargos'

    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title='Пошук вантажу')
        form = self.form_class(self.request.GET or None)
        context['form'] = form
        return dict(list(context.items())+list(c_def.items()))

    def get_queryset(self):
        queryset = Cargo.objects.order_by('-updated_at').filter(published=True, status='PENDING',freeze=False).exclude(
...
class MYCargo(LoginRequiredMixin, DataMixin, ListView):
    paginate_by = 50
    model = Cargo
    template_name = 'cargo/my_cargo.html'
    context_object_name = 'cargos'

    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title='Мій вантаж')
        return dict(list(context.items())+list(c_def.items()))

    def get_queryset(self):
        id_user = self.request.user.pk
        return Cargo.objects.order_by('-status','published','-updated_at' ).filter(author=id_user).prefetch_related('body_type','load_unload','permissions')

class Cargo_update(LoginRequiredMixin,DataMixin, UpdateView):
    model = Cargo
    form_class = CargoForm
    template_name = 'cargo/add_cargo.html'

    def get_object(self, queryset=None):
        cargo = super().get_object(queryset)
        if cargo.author != self.request.user:
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        c_def = self.get_user_context(title='Оновити Інформацію про вантаж')
        return dict(list(context.items())+list(c_def.items()))